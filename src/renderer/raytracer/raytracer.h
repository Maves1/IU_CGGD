...
namespace cg::renderer
{
			...
			const VB& vertex_a, const VB& vertex_b, const VB& vertex_c)
	{
		...
		ambient = float3{vertex_a.color_ambient.r, vertex_a.color_ambient.g,
						 vertex_a.color_ambient.b};
		diffuse = float3{vertex_a.color_diffuse.r, vertex_a.color_diffuse.g,
						 vertex_a.color_diffuse.b};
		emissive = float3{vertex_a.color_emissive.r, vertex_a.color_emissive.g,
						 vertex_a.color_emissive.b};
	}
...
//				 triangles.push_back(triangle);
			...
			float3 right, float3 up, size_t depth, size_t accumulation_num)
	{
		...
		{
			 std::cout << "Tracing frame #" << frame_id << std::endl;
			 ...
			 for (int x = 0; x < width; x++)
			 {
				 for (int y = 0; y < height; y++)
				 {
					 ...
					 ray curr_ray(position, ray_direction);

					 payload payload = trace_ray(curr_ray, depth);
